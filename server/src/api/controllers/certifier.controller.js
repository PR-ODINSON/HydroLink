...
async function approveCredit(req, res) {
  try {
    // ... your code to find credit and producer ...
    // Example:
    // const credit = await Credit.findById(req.params.id);
    // const producer = await Producer.findById(credit.producerId);

    // Mint the NFT on blockchain
    const mintResult = await mintCredit(producer.walletAddress, credit.metadataUri);

    if (!mintResult.success) {
      return res.status(500).json({ error: 'Blockchain minting failed', details: mintResult.error });
    }

    // Update credit document with blockchain info
